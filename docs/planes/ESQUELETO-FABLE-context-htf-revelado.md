...
| D | ✅ | S110: sección REVELADO añadida (revD1Up/Dn + revH1Up/Dn con histéresis, solo `barstate.isconfirmed`; cascada por chart-TF: chart D1=sin ctx, H1=solo D1, M5=D1+H1; flags show*). core-sync ×3 OK, pine_check 0/0, apply VIVO 22s sin CE10117/OOM. `reveal_bits=0`=DORMANT correcto (precio dentro rango D1; H1 no aplica en chart H1). Validación REVEALED-al-romper → Paso F | (commit S110) |
| E | ✅ | S111: `// === DIBUJO CONTEXTO ===` añadido. Slots FIJOS (var, precreados 1x: 8 boxes+8 lines+16 labels; en islast solo set_*, RE10045-safe). Estilo §5.4: hue familia (CTX_OB azul / CTX_FVG naranja / CTX_LIQ violeta), borde punteado, transp escalonada (D1 fill 92/borde 55/w3 · H1 90/45/w2 → nativo<H1<D1), tag "TF ▲/▼ concepto". Regla solape MTF (Tarea #2 S108 aprobada): heredado detrás (H1 tras D1 + Context bajo Visual), transp escalonada, colapso >70% rango menor→solo TF mayor (cajas: solape vertical; líneas: nivel dentro 0.3×ATR). Anclaje `xloc.bar_time` (patrón f_drawMTFZones). pine_check 0/0, core-sync ×3 OK. Apply VIVO 0 err sin CE10117/OOM | (commit S111) |
| F | ✅* | S111: matriz VIVA OANDA:EURUSD. **(1)** H1 `i_ctxAlways=false` → reveal_bits=0, 0 cajas visibles (8 slots ocultos bgcolor na) ✅. **(3)** `i_ctxAlways=true` H1 → reveal_bits=3 (solo D1; H1 suprimido en chart H1), caja aislada "D1 ▲ OB" azul punteada 1.17-1.18 anclada bar_time→now ✅. **(4)** M5 → reveal_bits=15 (4 flags D1+H1); visto D1▲OB (caja) + H1▲BSL + H1▼EQL (líneas violeta) simultáneos, colapso NO oculta (sin par redundante) ✅. **(5)** convivencia Visual+Context+LuxAlgo sin errores ✅. **(6)** 0 err apply, sin CE10117/RE/OOM ✅. **(7)** core-sync ×3 + pine_check 0/0 ✅. **(2) Ruptura histórica:** lado DORMANT confirmado (reveal_bits=0 precio dentro rango); lado REVEALED confirmado vía `i_ctxAlways` (misma ruta de dibujo). Reveal-al-romper en vivo NO reproducible en EURUSD hoy sin replay TV (=pago); máquina de frontera validada lógicamente S110 + transición 0→3→15 observada. `*`=Test 2 break-side diferido a un día con precio fuera del rango D1 o replay | (commit S111) |
