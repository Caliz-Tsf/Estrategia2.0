...
| 10 | **`chopratejas/headroom`** (viejo) → **`headroomlabs-ai/headroom`** (vivo, S077) | Compresión de contexto | **Diferir para enjambre; DECISIÓN USUARIO para Claude Code** | Solo si topan límites | **S077:** el que investigó la fila era `chopratejas/headroom`; el mantenido es OTRO: **`headroomlabs-ai/headroom`** (pip `headroom` CLI; `npm headroom-ai` = solo TS SDK). Ese SÍ apunta al **flujo Claude Code**: `headroom wrap claude` = proxy que comprime tool-outputs/logs/RAG/archivos/historial antes del LLM (claim 60–95%); se registra como MCP (`claude mcp add headroom -s user -e HEADROOM_PROXY_URL=...`). **CHOQUE:** el proyecto ya monta `context-mode` MCP (`.mcp.json:28`), mismo propósito → apilar ambos puede **encarecer** el 1er prompt (advertencia documentada). Para el enjambre Hermes sigue en DIFERIR (Hermes ya comprime). Para Claude Code = go/no-go del usuario, en trial controlado y **sin apilar con context-mode**. |
