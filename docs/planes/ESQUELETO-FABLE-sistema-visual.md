...
|---|---|---|---|
| 1 | Campos `strength` en UDTs (E.1/HANDOFF §5.1) | CORE | 0/0 ×3 + core-sync + SHA |
| 2 | `f_zoneStrength`/`f_eventStrength`/`f_strengthLevel` + cálculo en sitios de creación | CORE | ídem |
| 3 | Constantes `COL_*` centralizadas (§5.2) + matriz de render §5.3/§5.4 aplicada por familia | Visual | 0/0 Visual |
| 4 | `i_densidad` + `f_visualLevel` v2 (anclas §2.3 + pisos MTF) + matriz §8.2 | Visual | 0/0 |
| 5 | Reordenar bloques de dibujo a capas L0→L4 (§3) + KZ→ribbon + grid sin fill (§6.3) | Visual | 0/0 + checklist #4 |
| 6 | Consolidación de labels (§6.2) + alternancia + presupuesto global (§6.5) | Visual | 0/0 + checklist #1–3 |
| 7 | Estilo de procedencia MTF (§7.2) + pisos de herencia (§7.1) + 3 pendientes (§7.3, un commit c/u) | Visual | 0/0 + checklist #6–7 |
| 8 | Panel T14: fila Ocultos + glifos strength + modo (§9) | Visual | 0/0 + checklist #8 |
| 9 | Pasada de verificación completa §10 + Eje 2 (METODOLOGIA §3/§7) por concepto | — | ≥90 + firma usuario → **cierra el gate visual F1** |
