...
**Consumidor auxiliar opcional `[ADR-017]` (Fase 1, S110):** `SMC-Context.pine` (`indicator`, solo dibuja) es un **tercer consumidor** con el mismo `// === LIBRARY CORE ===` byte-idéntico (`scripts/check-core-sync.ps1` verifica los 3). Nace de un bloqueador doble: el transporte nearest-N del CORE no lleva los extremos HTF lejanos, y el Visual está pegado al techo CE10117. Hace su **2ª `request.security` por HTF** con `f_tfExtremes` (emite el extremo importante por concepto/lado, no nearest-N) **fuera** del CORE — presupuesto de tokens y memoria frescos por-script (viabilidad OOM validada vivo, gates A0/A2/A3 S109). No tocó Visual/Strategy/SHA en Fase A; reversible. **Fase B (ADR-018, S115) ejecutada:** `f_tfExtremes` + `f_farthestZone/Pool/Event` **promovidas al CORE byte-idéntico ×3** (single source of truth, ya no un 4º punto de verdad), Strategy añade 2ª `request.security` de extremos (D1+H1) para el scoring; SHA re-baselined `5510361166844bd5` → **`d86bf37aacbd25cf`** (CORE **1952 líneas**). GATE B (probe 4 securities) validó token/OOM en vivo antes de romper el SHA. Diseño: `docs/planes/DISEÑO-fase-B-promocion-core.md`; decisión: `docs/adrs/ADR-018-promocion-extremos-htf-al-core.md`.
...
13b. **Mapa MTF rico** — `N_htf` zonas/lado (OB/FVG/pool/sweep) en tuple plano capado (presupuesto ~127, `ESQUELETO-P3 §2.2`) + `f_nearestN` pura en CORE + reconstitución y dibujo en cascada con cuota de objetos. ✅ **COMPLETO & COMMITEADO (S041, commit 99ddeec)** — entrega un **mapa de niveles** (líneas extendidas al borde); geometría nativa → T13c.
13c. **Mapa MTF geométrico** ✅ **COMPLETO & COMMITEADO (S042, commit 88500fd)** `[ADR-010 · detalle en docs/sprint-runs/revision-T13b-mtf-dibujo-nativo.md]` — corrige T13b: la ranura crece a **6 campos** `[kind,top,bottom,dir,tA,tB]` (añade anclaje temporal) y el consumidor **reconstruye la forma NATIVA de cada concepto anclada por `xloc.bar_time`** (OB/FVG=caja, EQH/EQL=línea tramo, BOS/CHoCH=línea origen→ruptura, sweep=marca ▲▼ en su barra, pool=línea). **Cap 0-10 por concepto para TODOS los conceptos** (no solo OB/FVG/pool); buffer genérico etiquetado de ADR-009 conservado (solo crece la ranura). Supersede el transporte de ADR-009. Compila 0/0 los 3, core-sync OK SHA 4dc885e1bd23ec8b (926 líneas), validación visual EURUSD M5 ✅. RE10020 corregido (xloc.bar_time en todas las etiquetas). Pendientes diferidos: tag TF en BOS/CHoCH fusionados, cuadre EQH/EQL, exactitud FVG/OB vela-a-vela (notas follow-up S042). Ver [[Sesion-042]].
14. ✅ **COMPLETO & COMMITEADO (S043)** Panel de estado completo (multi-columna: Concepto | D1 | H1 | chart-TF). 100% Visual (CORE intacto, core-sync `88cbb03e...`/929). 4 columnas pobladas desde `d1State`/`h1State` + buffer geométrico MTF (D1/H1) y arrays nativos (chart-TF); header de columna propia dinámico (`f_tfLabel`); conceptos solo-chart (Bias dom. 50, Kill Zone) muestran "—" en D1/H1. Texto `size.small`; toggle `i_panelCompact` = minimizado a header+Bias (2 filas); `i_showPanel` apaga el panel. Compila 0/0, validado visual EURUSD M5. Ajustes del panel anotados para Opus Ultracode (S043 §Pendientes).
...
**Sprint 1.5 — Tier 2:** `[EN CURSO · S045]`
16. ✅ Displacement (`f_detectDisplacement` puro §4.1, S045 commit b870218; MSS refactorizado DRY; validación ≥90 pendiente) → 17. ✅ IDM (`f_detectIDM` grab liquidez interna §3.6, S045 commit ad94b61; validación ≥90 pendiente) → 18. Judas → 19. Breaker → 20. Rejection → 21. Flip → 22. OTE + Golden Pocket → 23. EMAs (estado, cruces, rebotes) → 24. False Breakout → 25. Impulsive/Corrective
