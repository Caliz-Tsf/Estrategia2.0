...
13c. **Mapa MTF geométrico** `[NUEVO · Sesion-041 · ADR-010 · detalle en docs/sprint-runs/revision-T13b-mtf-dibujo-nativo.md]` — corrige T13b: la ranura crece a **6 campos** `[kind,top,bottom,dir,tA,tB]` (añade anclaje temporal) y el consumidor **reconstruye la forma NATIVA de cada concepto anclada por `xloc.bar_time`** (OB/FVG=caja, EQH/EQL=línea tramo, BOS/CHoCH=línea origen→ruptura, sweep=marca ▲▼ en su barra, pool=línea). **Cap 0-10 por concepto para TODOS los conceptos** (no solo OB/FVG/pool); buffer genérico etiquetado de ADR-009 conservado (solo crece la ranura). Supersede el transporte de ADR-009.
