...
- **El conteo tampoco es aditivo:** el mismo `fix-slim` mide **45** sobre `lastre(300)` y **27** sobre `lastre(400)` ⇒ las restas arrastran **~18 tokens** de dependencia del contexto.
- **PRECISIÓN DEL INSTRUMENTO (calibrada en S138):** intercepto ±1246 tokens · restas ±18 tokens. ⇒ **el lastre sirve para señales >100 tokens. Por debajo de eso, y para cualquier valor absoluto, NO.** Los hallazgos grandes de la campaña quedan holgados (código muerto = 0 vs 700 predichos; `ΔT`=142 vs 830).
- **Cómo acotar un absoluto sin extrapolar — BRACKET:** combinar el hecho de que el build limpio aplica (`≤ límite`) con una lectura directa por encima del techo. Ej. S138: `Visual` aplica ⇒ `base_V ≤ 100256`; `Visual+fix` = 100262 con `coste(fix) ∈ [27,45]` ⇒ `base_V ∈ [100217, 100235]` ⇒ **headroom ∈ [21,39]**. Da un intervalo honesto en vez de un número falso.
- **Cuándo la extrapolación aún sirve:** cuando el error (~1,2k) es pequeño **frente a la respuesta**. `Context ≈ 85.000 libres` (S135) ⇒ error del 1,5% ⇒ se mantiene como **~83.000 ± 3.000**, usable. La misma extrapolación es inservible para "¿39 o 1285?", donde el error es del orden de la respuesta.
