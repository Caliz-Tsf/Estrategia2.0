...
| **I-0** | ✅ **HECHO Y VERIFICADO EN VIVO (S145, commit `842d8db`, TV v390 "Compilado. / Añadido al gráfico.")** — marcador `▲ BSL 1.51421 ·3` dentro de banda, la escala deja de estirarse a 1.54. **Compila bajo el límite de tokens** (TV solo imprime la cifra al fallar ⇒ hay veredicto, no número exacto). **Paso 1, Visual-only, sin tocar el CORE**: anclas `poolTopIdx`/`poolBotIdx` (alto más alto / bajo más bajo) + puerta de banda `[dolLo, dolHi]` + marcador de borde, financiado retirando el top-2 por `strength` y la lógica `aligned/drawFar`. `check-core-sync` OK (SHA `7ad95b3e612d041a`, sin cambios), `pine_check` 0/0. | Saldo de tokens con **apply real** (predicción escrita antes de medir: ahorro neto 20-60). |
