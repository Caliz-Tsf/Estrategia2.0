...
- **⚠️ PRIORIDAD #1 PRÓXIMA SESIÓN (pedido de Freddy):** **verificar que las 9 skills y los 8 agentes están bien creados y FUNCIONANDO/activos al necesitarlos** (= gate VER-04 + prueba Done de Bloque E). Pasos: (1) al arrancar en frío, listar y confirmar 9 skills invocables + 8 agentes en el registro del tool `Agent`; (2) invocar `smc-code-explorer` con pregunta sobre `pine/reference/LuxAlgo-SMC-base.pine` → debe responder con archivo:línea; (3) probar `smc-session-startup` end-to-end y ≥1 agente más; (4) si algo no registra, diagnosticar (carpeta vs `name:`, frontmatter, campo `tools`). **Motivo:** los agentes creados en Sesion-006 NO se registraron en caliente (el registro de subagentes carga al arrancar la sesión); las skills sí. Hay que confirmarlo en frío.
- **SIGUIENTE PASO (tras la verificación):** Bloque F (WF-01 smc-sprint.yaml + scripts restantes SCR-01/03/04; SCR-02 ya hecho) → VER-01..08 → gate VER-09 REVISIÓN-FABLE. (Bloques D y E ✅ construidos en Sesion-006.)
