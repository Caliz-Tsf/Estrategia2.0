...
- [ ] **MCP-06** task-master parse — Claude Code → `task-master parse-prd`. ⏳ **MOVIDO A VER-09 (decisión usuario Sesion-005):** NO se genera backlog ahora (divergiría del workplan vivo). Se ejecuta DENTRO del gate VER-09: tras construir todo (Bloques D/E/F) y que **Fable revise** que skills+agentes+MCP+workflows estén bien, se corre task-master sobre el workplan ya validado para producir el plan de escritura concreto de Fase 1. CLI disponible (0.43.1). Done: backlog generado post-revisión-Fable.
