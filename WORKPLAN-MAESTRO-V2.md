...
- [x] **SCR-04** `scripts/process-video.ps1` — Claude Code → ✅ **Sesion-007**. Pipeline yt-dlp → FFmpeg (WAV 16kHz mono) → Whisper (openai-whisper) → `.md` con frontmatter al vault `Teoria SMC`. Soporta `-Url` o `-InputFile`, `-Model tiny..large`, `-Language` (autodetecta si se omite), `-MaxSeconds` (recorte para tests). ASCII-only (.ps1); `.md` de salida UTF-8 con acentos. Done OK: probado end-to-end con video real de 19s → transcripción real + nota correcta (exit 0). ⚠️ opcional, no bloquea el gate. Fix **Sesion-018**: VaultRoot default corregido a `D:\obsidian\boveda MENTE\Mente` + SubFolder `Teoria SMC` (antes apuntaba a `Estrategia2.0\Teoria-SMC`). Docs: `docs/VIDEO-CURSOS.md`.
...
- [x] **F1-S1.1** Fundación: **[x] T01 esqueleto 3 archivos + UDTs + arrays acotados (Sesion-011, commit 45449f3, compile 0/0, core sync OK)** · **[x] T02 swings+clasificación (Sesion-012, commit aa4ed23, compile 0/0, core sync OK, smc-validator 97/100)** · **[x] T03 BOS+CHoCH (Sesion-013, commit abc5988, compile 0/0, core sync OK 207 líneas, smc-validator 97/100)** · **[x] T04 bias por TF (Sesion-014, commit 3dff0a6, compile 0/0, core sync OK 227 líneas, smc-validator 97/100).** Done: validados ≥90 + commit cada uno (+ paridad ver05 en T02/T03).
- [ ] **F1-S1.2** Zonas: **[x] T05 OB+mitigación (Sesion-016, commit 698b9dd, compile 0/0, core sync OK 316 líneas, smc-validator 97/100, 3 casos §2.1 exactos).** · **[x] T06 FVG+CE+mitigación explícita (Sesion-017, commit 8c5f88f, compile 0/0, core sync OK 342 líneas, smc-validator 95/100, 3 casos §2.2 exactos).** · **[x] T07 Premium/Discount/Equilibrium (Sesion-021, commit ddd8cc5, compile 0/0, core sync OK 415 líneas SHA 24c3a4a2, smc-validator 97/100, 4 niveles §2.3 exactos vs LuxAlgo; trailing extremes opción A, swing P/D=50, dibujo en líneas; docs/decisiones-pd-rango.md).** · **[x] T07B Estructura BOS/CHoCH dominante swing=50 (Sesion-022, commit a9c98de, compile 0/0, core sync OK 415 líneas SHA 24c3a4a2, visual 3 casos dominantes confirmados, capa aditiva, dibujo BOS+/CHoCH+ línea width 3 etiquetas centradas, ADR-004).** · **[x] T08 EQH/EQL (Sesion-026, commit 0b63eda, compile 0/0, core sync OK 437 líneas SHA 2c6b47b, smc-validator 97/100, 3 casos §2.4 exactos; escala 3, umbral 0.1×ATR, array SMC_eqhl dedicado, dibujo azul punteada, alimenta pools liquidez Sprint 2.1).** (+ paridad ver05 en T05/T06/T08.) · T09 siguiente. **Siguiente fase: F1-S1.3 Liquidez.**
- [x] **F1-S1.3** Liquidez ✅ COMPLETADO: **[x] T09 pools (Sesion-027, commit 139e1c1, compile 0/0, core sync OK 503 líneas SHA 2949a93, f_buildPools clustering EQH/EQL+swings, visual BSL/SSL dashed naranja/aqua, bug fix FIFO→push, decision mitigación+swept diferido Opus Max).** · **[x] T09b/T10 cierre formal + ciclo de vida (Sesion-030, commit 45c4b40, core sync OK 592 líneas SHA 8967d31, ADR-006 pools persistentes+swept mark, validación ≥90 APROBADO 93/100).** · **[x] T11 Kill Zones (Sesion-035, commit 68f2946, compile 0/0, core sync OK 623 líneas SHA 4da142bc, validación 95/100, f_killZone + perfiles FX-London-NY/FX-Asia, wiring confluencia #34).** · **[x] T12 MSS — COMPLETO (Sesion-036, commits 114a62b/4b4f3cc/88c8e92/f561b34, compile 0/0, core sync OK 644 líneas SHA 2de06be3, validación 95/100, 14 MSS swing en histórico ≥3 criterio VER-09, f_detectMSS puro, unificación CHoCH/MSS, tooling pine_check/pine_inject, panel reescrito, consolidación f_drawStructure).**
- [x] **F1-S1.4** MTF+panel ✅ COMPLETADO: **[x] T13 snapshots D1/H1 + dibujo MTF diferenciado (T13a/T13b/T13c, Sesion-040/041/042, d6a5418/99ddeec/88500fd, core sync OK 853→929 líneas, ADR-010 mapa geométrico nativo anclado).** · **[x] T14 panel de estado completo (Sesion-043, 7854ccf, CORE INTACTO, 4 columnas D1/H1/chart-TF, helpers Visual-only, ajustes tamaño/minimizado).** · **[x] T15 alertas SMC completas (Sesion-044, 5a1d4b9, CORE INTACTO, 23 alertcondition() cobertura sprint, anti-repaint D-PINE-03, 0/0 compilador dual).** **Gate parcial Tier 1 → SIGUIENTE: validación visual ≥90 T01–T12 + anti-repaint 2 días + perf 20k barras.**
- [x] **F1-S1.5** Tier 2 ✅ COMPLETADO: **[x] T16 displacement (Sesion-045, b870218, CORE 949 líneas, validate ≥90 PENDIENTE).** · **[x] T17 IDM (Sesion-045, ad94b61, CORE 949, validate ≥90 PENDIENTE).** · **[x] T18 Judas (Sesion-046, fbef02b, f_detectJudas composición Sweep+Displacement, compile 0/0, core sync OK 1245 líneas).** · **[x] T19 breaker (Sesion-046, df5b108, f_detectBreaker OB→invalidación→zona invertida).** · **[x] T20 rejection (Sesion-046, 5bc4828, f_detectRejection geometría mecha + anclaje).** · **[x] T21 flip (Sesion-046, dc8bfa6, f_updateFlip nivel cambio de rol).** · **[x] T22 OTE+GP (Sesion-046, 028c703, f_detectOTE zonas Fib sobre BOS).** · **[x] T23 EMAs estado/cruces (Sesion-046, 4cdc825, f_emaState 6 confluencias unificadas).** · **[x] T23b Stack Flip (Sesion-046, 95b0af3, cruce 20/50 sobre 200 = régimen; gate regla §4.3 ampliada).** · **[x] T24 false breakout (Sesion-046, 7172d85, f_detectFalseBreak + f_classifySweep Spring/Raid).** · **[x] T24b mitigation block (Sesion-046, 08c717f, cierre §2.8).** · **[x] T25 impulsive/corrective (Sesion-046, 7893b55, f_classifyLeg; gate regla §4.4 cuantificada pre-código ✅).** **Done:** 0/0 compilador (todos) · core-sync OK 1245 líneas SHA `c8603f808a6161df` · regla §4.4 completa + casos verificados · **Validación visual smc-validator-agent T16–T25 APLAZADA próxima sesión usuario presente.** **DECISIÓN SPRINT 1.6:** Gate "reglas antes de código" — NO codificar T26–T40 sin regla cuantificada (umbrales + casos EURUSD) en reglas-smc-ict.md.
- [x] **F1-S1.6 Fase 0 (reglas)** ✅ COMPLETADO (Sesion-047, 2026-06-21): **§5 GAP ICT — 15 fichas T26–T40 redactadas en `docs/reglas-smc-ict.md`** con umbrales cuantificados relativos a ATR, casos EURUSD H1 verificados, contraejemplos, parámetros default. Gate "reglas antes de código" CUMPLIDO. **ADR-011 SMT símbolo correlacionado ✅ ACEPTADO** (default EURUSD↔GBPUSD positiva, input `i_smtSymbol` configurable, patrón request.security aplanado, honra ADR-001, desbloquea T38). CORE 1245 líneas SHA `c8603f808a6161df`, compila 0/0, core-sync OK. **5 commits:** reglas lotes 1-3 (2ec981f/b43a744/695b68d) + ADR-011 propuesta+aceptado (4cc4603/0a70909). **Próxima: codificación T26–T40 con pipeline 7 etapas.** Gap de conceptos ICT (primitivas nuevas). Integra los conceptos que enseñan los mentores y el Pine aún no detecta (`docs/planes/MATRIZ-conceptos-cobertura.md`), por el **mismo pipeline de 7 etapas**. Esqueleto maestro: `docs/planes/ESQUELETO-P1-conceptos-EA-pine-repos.md`. **Solo las primitivas de detección (Rutas A y D) son Fase 1**; sesgo (B)→Sprint 2.1, modelos de entrada (C)→motor EA Fase 4, noticias (E)→gate determinista Fase 4/5, teorías (F)→bucket experimental post-Fase 3. Orden por palanca:
  - **Ruta A (primitivas):** T26 True FVG (*variante §2.2, refina confluencia, no suma #*) · T27 IFVG · T28 BPR · T29 Immediate Rebalance · T30 serie gaps de apertura NWOG/NDOG/NYMO/ORG + Breakaway Gap (*extiende §4.2, no re-detecta el nivel Session Opens*) · T31 Volume Imbalance (+SIVI/BIVI) · T32 CISD (*familia estructura — spec DEBE distinguirlo de §1.4 CHoCH/§1.5 MSS*) · T33 Vacuum Block · T34 Propulsion Block (*variante §2.1 OB, refina*) · T35 IPR · T36 Standard Deviation (*herramienta SL/TP, NO confluencia*) · T37 Inside Day · T38 SMT Divergence (**precede ADR de símbolo correlacionado**).
  - **Ruta D (tiempo/sesión, extiende §3.4):** T39 macros intradía (NY Lunch / Opening Range / NY Lunch Hour) · T40 RTH vs ETH.
  - **Anti-duplicados (cruce Sesion-034):** cero duplicados exactos vs §1–§4; 5 guards de delineación obligatorios → `ESQUELETO-P1 §2.1`. **Confluencias:** ~8–9 candidatas nuevas #43–#51 (`ESQUELETO-P1 §5`); número y pesos finales = F2-T01 / Fase 3. **Decisiones:** ADR-011 SMT ✅ ACEPTADO (Sesion-047), Standard Deviation = herramienta SL/TP (T36) — sin ADR. ⚠️ **SIGUIENTE:** Codificación Sprint 1.6 (T26–T40) — orden recomendado en Sesion-047, familia FVG→gaps→estructura→propias→herramienta→SMT→sesión. Checkpoint T26 completado antes de series.
- [x] **F1-S1.6 Fase 1 (codificación) ✅ COMPLETADO (Sesion-048, 2026-06-21)** — **15 conceptos T26–T40 CODIFICADOS** siguiendo ciclo: spec cuantificada ✅ Sesion-047 → `f_detect*` puro en CORE → 0/0 compilador (los 3 archivos) + core-sync OK byte-idéntico (1517 líneas SHA `80fad14dd8d03758`) → 15 commits (1 por concepto). **Commits:** T26 (8c4f5ef) · T27 (cf343e9) · T28 (596d045) · T29 (a820975) · T31 (0a33a9d) · T30 (055a009) · T32 (f331b88) · T34 (f381f40) · T33 (1c83851) · T35 (2370788) · T37 (a665c92) · T36 (6898af0) · T39 (95a7078) · T40 (644bd65) · T38 (617375e). **Decisiones de diseño:** refinamientos (True FVG, Propulsion) = flags SMC_Zone (no confluencias nuevas, evita P-05); Volume Imbalance = array SMC_vi dedicado; BPR dirección por retest; StdDev = herramienta SL/TP sin confluencia #; core-sync divergente T30 detectado+corregido. **Validación visual smc-validator-agent (Tier 2+3):** APLAZADA próxima sesión usuario presente. **Próxima:** validación + F1-GATE. SIN git tag aún (gate pendiente).
- [x] **F1-GATE AMPLIADO ✅ VALIDACIÓN COMPLETADA (Sesion-057, actualizada S099 2026-07-06)** — Validación visual 1-a-1 completada 40 conceptos (T01–T40) + #41 MTF + rediseño Fase A A-2…A-7: **Tier 1 96/100 · Tier 2 94/100 · Liquidez 95/100 · Contexto/EMAs 6/6 · GAP ICT 93/100** · panel ✅ · **anti-repaint ✅ 2 días histórico EURUSD** · **perf ✅ 20k barras** · presupuesto objetos OK · **Ejes 0/1/3 VALIDADAS, Eje 2 (fuerza) en paralelo**. **(S099 familia-por-familia completa):** densidad D1 17/H1 24/M5 23 labels ≤25/≤60 objetos ✅; Ocultos/Desalojo §10-#8 ✅; glifo strength/confDegree ✅; FLIP promocionado ✅. **🔴 BLOQUEANTE:** BUG #1 detectado (zonas nativas OB/FVG no dibujan Operación, solo MTF-heredadas; root cause band-pick §A-4). **Entregable:** `docs/planes/BUGS-VISUAL-fable-revision.md` (commit `339f748`) — BUG caracterizado + restricciones fix (anti-repaint/CORE byte-idéntico/RE10045-safe/ADR-002 intacto). **FIRMA F1-GATE BLOQUEADA por BUG #1** (pendiente root cause + fix Fable). Metodología `METODOLOGIA-VERIFICACION-VISUAL.md` + `docs/sprint-runs/VALIDACION-Sesion-057-resultados.md` · CORE intacto 1700 líneas SHA `5510361166844bd5` · **SIN git tag aún.** (Fable review + bug fix → firma).
...
>
> **Expansión Sprint 1.7 `[Sesion-080]` (Gradient Levels):** una sola confluencia candidata nueva, **#52 = confluencia exponencial nivel∩quadrant** (T42), que **NO es un sumando plano sino un MULTIPLICADOR** del score direccional: `scoreDir_final = scoreDir_raw × (1 + gradientBonus)` — ver **ADR-013** (cambia el contrato de `f_scoreConfluences` de aditivo a aditivo+multiplicativo, wiring en F2-T01). T41 (grid del rango) es **herramienta** (no confluencia, §0.9); T43 (FVG válido por gradient) **refina #18/#20** vía flag `gradedFvg` (no suma #); T44 (grading de mecha) = herramienta opcional en backlog. `k`/`gradMultMax` se calibran en Fase 3. Detalle: `docs/planes/ESQUELETO-P4-gradient-levels.md` + `reglas-smc-ict.md §5B`.
