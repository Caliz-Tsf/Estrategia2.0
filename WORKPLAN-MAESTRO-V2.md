...
- [ ] **F1-S1.6** `[NUEVO — Sesion-032 planificación]` Integración de 24 conceptos faltantes + 12 variantes — **investigados vs cursos mentores `docs/planes/MATRIZ-conceptos-cobertura.md`** — divididos en 6 rutas (A primitiva Pine, B sesgo, C modelo-entrada, D tiempo-macro, E noticias-gate, F teoría) en esqueleto maestro `docs/planes/ESQUELETO-P1-conceptos-EA-pine-repos.md`. **T26–T40 ordenado por palanca**, ciclo canónico 7 etapas (spec → f_detect* → 0/0 → validación ≥90 → confluencia §4.8 → golden test MQL5 → commit). **Decisiones pendientes:** SMT Divergence símbolo-referencia (ADR candidato), Standard Deviation (herramienta SL/TP, no confluencia). Relleno del esqueleto → sesión próxima (implementación paralela código Pine). ⚠️ **SIN checkpoint** hasta T26 primera completado.
