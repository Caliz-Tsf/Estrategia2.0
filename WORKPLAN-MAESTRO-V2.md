...
- [ ] **MCP-02** Sync fork TV MCP con upstream — usuario/Claude Code → `git fetch upstream && git merge` en `D:\CODE\BOT\Bot\tradingview-mcp-jackson`. Done: `tv_health_check` OK, 81 tools.
- [ ] **MCP-03** Instalar Archon v2 — usuario/Claude Code → `git clone https://github.com/coleam00/Archon .archon`. Done: 18 workflows default visibles. ⚠️ Si la instalación falla: los workflows se ejecutan manualmente (WORKFLOWS.md, regla de fallback) y el proyecto NO se bloquea.
- [ ] **MCP-04** Instalar/actualizar claude-mem — usuario → `npx claude-mem install`, settings con puerto 37777. Done: dashboard responde. ⚠️ no-bloqueante.
- [ ] **MCP-05** `rules.json` (morning_brief) — Claude Code → watchlist EURUSD, criterios bullish/bearish/neutral H1, risk rules, TFs D/60/5 (contenido base del plan original `[✓ MANTENER]`). Done: `morning_brief` devuelve bias EURUSD.
- [ ] **MCP-06** task-master parse — Claude Code → `task-master parse-prd WORKPLAN-MAESTRO-V2.md` para backlog. Done: backlog generado. ⚠️ opcional.

### Bloque D — Skills (contenido completo en SKILLS.md — copiar tal cual)
- [ ] **SKL-01..09** Crear las 9 skills en `.claude/skills/<nombre>/SKILL.md` — Claude Code → contenido del anexo. Done: las 9 aparecen como invocables; `/smc-session-startup` ejecuta sin error.

### Bloque E — Agentes (contenido completo en AGENTES.md — copiar tal cual)
- [ ] **AGT-01..08** Crear los 8 agentes en `.claude/agents/<nombre>.md` — Claude Code → contenido del anexo. Done: los 8 listados; prueba: invocar smc-code-explorer con una pregunta sobre el LuxAlgo base responde con archivo:línea.

### Bloque F — Workflows y scripts
- [ ] **WF-01** `.archon/workflows/smc-sprint.yaml` — Claude Code → YAML de WORKFLOWS.md adaptado al schema del Archon instalado. Done: Archon lo lista (o queda como doc ejecutable si Archon ⚠️).
- [ ] **SCR-01** `scripts/launch-tv-agent.ps1` — Claude Code → mejora del existente: fix TV v2.14+, auto-load SMC-Visual, tv_health_check al arrancar. Done: lanza TV con el indicador cargado.
- [ ] **SCR-02** `scripts/sync-obsidian.ps1` — Claude Code → copia memory/sesiones/ + docs/adrs/ nuevos al vault `D:\obsidian\boveda MENTE\Mente\Estrategia2.0\`. Done: dry-run correcto.
- [ ] **SCR-03** `scripts/check-core-sync.ps1` `[NUEVO]` — Claude Code → extrae la sección `// === LIBRARY CORE ===` de SMC-Visual.pine y SMC-Strategy.pine y compara hash; exit 1 + diff si divergen. Done: detecta una divergencia inyectada de prueba.
- [ ] **SCR-04** `scripts/process-video.ps1` — Claude Code → pipeline yt-dlp → FFmpeg → Whisper → .md al vault Teoría SMC. Done: procesa 1 video de prueba. ⚠️ opcional, no bloquea el gate.
...
- [ ] **F1-S1.2** Zonas: T05 OB+mitigación · T06 FVG+CE+mitigación explícita · T07 Premium/Discount · T08 EQH/EQL.
- [ ] **F1-S1.3** Liquidez: T09 pools · T10 sweeps+grabs · T11 Kill Zones (convención DST de P-25) · T12 MSS.
...
- [ ] **F1-S1.5** Tier 2: T16 displacement · T17 IDM · T18 Judas · T19 breaker · T20 rejection · T21 flip · T22 OTE+GP · T23 EMAs (estado/cruces/rebotes) · T24 false breakout · T25 impulsive/corrective.
