...
- [ ] **MCP-03** Instalar Archon v2 — usuario/Claude Code → `git clone https://github.com/coleam00/Archon .archon`. Done: 18 workflows default visibles. ⚠️ Si la instalación falla: los workflows se ejecutan manualmente (WORKFLOWS.md, regla de fallback) y el proyecto NO se bloquea. ⏳ **DIFERIDO Sesion-004** (no-bloqueante; pesado, requiere docker — se hará en Bloque F si se necesita).
...
- [ ] **MCP-06** task-master parse — Claude Code → `task-master parse-prd WORKPLAN-MAESTRO-V2.md` para backlog. Done: backlog generado. ⚠️ opcional. ⏳ **DIFERIDO Sesion-004** (opcional; el workplan sigue siendo la fuente de verdad — un backlog paralelo podría divergir).
