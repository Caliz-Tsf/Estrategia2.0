...
- [ ] **F1-S1.6** `[NUEVO — oficializado Sesion-034]` Gap de conceptos ICT (primitivas nuevas). Integra los conceptos que enseñan los mentores y el Pine aún no detecta (`docs/planes/MATRIZ-conceptos-cobertura.md`), por el **mismo pipeline de 7 etapas** (spec cuantificada → `f_detect*` puro → 0/0 + core-sync → validación ≥90 → confluencia §4.8 → golden test MQL5 → commit). Esqueleto maestro: `docs/planes/ESQUELETO-P1-conceptos-EA-pine-repos.md`. **Solo las primitivas de detección (Rutas A y D) son Fase 1**; sesgo (B)→Sprint 2.1, modelos de entrada (C)→motor EA Fase 4, noticias (E)→gate determinista Fase 4/5, teorías (F)→bucket experimental post-Fase 3. Orden por palanca:
